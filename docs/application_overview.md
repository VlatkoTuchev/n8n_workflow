...
- System prompt (server‑side): Loaded from exported per‑user prompt files at `/realtime/token`. The server reads `scripts/exports/user_prompts/<email>.txt` (generated by `scripts/export_user_prompts.js`) and uses its full contents as the session’s baseline instructions. This removes per‑request SQL and relies on the pre‑generated snapshot that already includes the static rules, language policy, user context summary, working memory, recent summary, and the last conversation anchor.
...
- Generate/refresh files: `npm run export:user_prompts`
- Source used by server: `server.js` → `/realtime/token` reads `scripts/exports/user_prompts/<email>.txt`
- Fallback: If a per‑user file is missing, the server falls back to a minimal onboarding‑first instruction set and reports `hasHistory: false`.
